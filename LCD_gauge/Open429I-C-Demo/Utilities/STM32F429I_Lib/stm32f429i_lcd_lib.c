...
//   LCD_PowerOn();
  ...
  LTDC_InitStruct.LTDC_TotalHeigh =3 + LCD_PIXEL_HEIGHT+204;
...
void LCD_Clear(uint16_t Color)
{
  ...
  /* erase memory */
  for (index = 0x00; index <BUFFER_OFFSET ; index++)  //76800 BUFFER_OFFSET 
  {
...
/*
 ...
 +                       LCD pins assignment                                   +
 +------------------------+-----------------------+----------------------------+
 |  LCD_TFT R0 <-> PH.02  |  LCD_TFT G0 <-> PE.05 |  LCD_TFT B2 <-> PE.04      |
 |  LCD_TFT R1 <-> PH.03  |  LCD_TFT G1 <-> PE.06 |  LCD_TFT B3 <-> PG.12  
